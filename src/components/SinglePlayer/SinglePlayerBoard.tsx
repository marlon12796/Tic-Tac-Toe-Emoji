...
import { type emojis } from '../../utils/types'
import { calculateSinglePlayerWinner } from '../../utils/caculateWinner'

export default function SinglePlayerBoard() {
  const { playerSign, board, setBoard, setGameResult, isHardMode } = useSinglePlayer()
  ...
  useEffect(() => {
    ...
    if (currentPlayer === '🤖' && !winner && gameNavigate === 'single-player-board') {
      setIsBotTurn(true)
  ...
  }, [gameNavigate, isPlayerXNext, playerSign, winner])
  ...
  const handleClick = (index: number) => {
    ...
    if (newWinner) {
      setWinner(newWinner)
      newWinner === playerSign ? setGameResult('win') : setGameResult('lose')
